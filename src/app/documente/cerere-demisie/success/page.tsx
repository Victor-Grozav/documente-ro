...
function SuccessContent() {
  ...
  const [data, setData] = useState<CerereDemisieData | null>(null);

  useEffect(() => {
    if (!sessionId) { setStatus("error"); return; }
    const raw = localStorage.getItem("cerereDemisieData");
    ...
    fetch(`/api/verify-session?session_id=${sessionId}`)
      ...
      .then(({ paid }) => {
        ...
        try {
          setData(JSON.parse(raw) as CerereDemisieData);
          setStatus("success");
        } catch {
          setStatus("error");
        }
    ...
    return (
      <main className="min-h-screen flex items-center justify-center px-4">
        <div className="max-w-md w-full bg-white dark:bg-slate-900 rounded-2xl border border-red-200 dark:border-red-800 p-8 text-center">
          ...
          <a href="/documente/cerere-demisie" className="text-blue-600 font-medium hover:underline text-sm">
            ← Încearcă din nou
          </a>
        </div>
      </main>
      ...
      <div className="max-w-lg w-full text-center">
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-green-200 dark:border-green-700 p-8 mb-4">
          ...
          <p className="text-gray-500 dark:text-slate-400 mb-6">
            Cererea de demisie este gata. Tipărește-o, semnează-o și înmânează-o angajatorului.
          </p>
          <PDFDownloadButton
            document={<CerereDemisie data={data} />}
            fileName={`cerere-demisie-${data.angajatNume.replace(/\s+/g, "-").toLowerCase()}-${data.data.replace(/\./g, "-")}.pdf`}
            label="Descarcă Cerere Demisie PDF"
          />
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-700 p-6 text-left">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              <span className="text-gray-500 dark:text-slate-400">Angajator</span>
              <span className="font-medium dark:text-white">{data.angajatorNume}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500 dark:text-slate-400">Preaviz</span>
              <span className="font-medium dark:text-white">{data.preavizZile} zile</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500 dark:text-slate-400">Ultima zi</span>
              <span className="font-medium dark:text-white">{data.dataUltimaZi}</span>
            </div>
          </div>
        </div>
