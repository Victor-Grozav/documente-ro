...
function SuccessContent() {
  ...
  const [data, setData] = useState<CerereConceduData | null>(null);

  useEffect(() => {
    if (!sessionId) { setStatus("error"); return; }
    const raw = localStorage.getItem("cerereConceduData");
    ...
    fetch(`/api/verify-session?session_id=${sessionId}`)
      ...
      .then(({ paid }) => {
        ...
        try {
          setData(JSON.parse(raw) as CerereConceduData);
          setStatus("success");
        } catch {
          setStatus("error");
        }
    ...
    return (
      <main className="min-h-screen flex items-center justify-center px-4">
        <div className="max-w-md w-full bg-white dark:bg-slate-900 rounded-2xl border border-red-200 dark:border-red-800 p-8 text-center">
          ...
          <a href="/documente/cerere-concediu" className="text-blue-600 font-medium hover:underline text-sm">
            ← Încearcă din nou
          </a>
        </div>
      </main>
      ...
      <div className="max-w-lg w-full text-center">
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-green-200 dark:border-green-700 p-8 mb-4">
          ...
          <p className="text-gray-500 dark:text-slate-400 mb-6">
            Cererea de concediu este gata. Tipărește-o și depune-o la angajator.
          </p>
          <PDFDownloadButton
            document={<CerereConcediu data={data} />}
            fileName={`cerere-concediu-${data.angajatNume.replace(/\s+/g, "-").toLowerCase()}-${data.data.replace(/\./g, "-")}.pdf`}
            label="Descarcă Cerere Concediu PDF"
          />
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-700 p-6 text-left">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              <span className="text-gray-500 dark:text-slate-400">Perioadă</span>
              <span className="font-medium dark:text-white">{data.dataInceput} – {data.dataSfarsit}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500 dark:text-slate-400">Număr zile</span>
              <span className="font-medium dark:text-white">{data.nrZile} zile</span>
            </div>
          </div>
        </div>
